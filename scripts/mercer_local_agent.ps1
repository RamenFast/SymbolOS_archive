...
function Get-MercerLocalContext {
  ...
  # Uses .NET ReadAllText for proper UTF-8 emoji handling (PS 5.1 Get-Content mangles multi-byte).
  # Falls back to a minimal hardcoded prompt if snapshot doesn't exist.
  if (Test-Path $contextFile) {
    try {
      $snapshot = [System.IO.File]::ReadAllText($contextFile, [System.Text.Encoding]::UTF8)
...
function ConvertTo-Utf8Json {
  # Manual JSON builder that preserves emoji/Unicode without PS 5.1 mangling.
  # Handles the specific shapes we send to llama.cpp chat completions.
  Param([hashtable]$Object)

  function Escape-JsonString([string]$s) {
    # JSON-escape a string, correctly handling supplementary plane chars (emoji).
      ...
      switch ($c) {
        ...
        default {
          ...
          if ($code -lt 0x20) {
            ...
            # Emit surrogate pair as-is (valid UTF-8 when encoded)
            [void]$sb.Append($c)
            $i++
            [void]$sb.Append($s[$i])
          } else {
  ...
  # Build full body
  $tempStr = $Object.temperature.ToString([System.Globalization.CultureInfo]::InvariantCulture)
  return "{`"messages`":$msgsJson,`"temperature`":$tempStr,`"max_tokens`":$($Object.max_tokens),`"stream`":false,`"chat_template_kwargs`":{`"enable_thinking`":false}}"
}

function Invoke-LocalLlm {
  # Uses .NET HttpWebRequest with explicit UTF-8 encoding to bypass PS 5.1's
  # broken ConvertTo-Json/Invoke-RestMethod which mangle supplementary-plane
  # Unicode chars (emoji above U+FFFF like  etc.).
  Param(
    [string]$System,
    [string]$UserMessage,
    [int]$MaxTokens = 1024,
  ...
  $jsonBody = ConvertTo-Utf8Json -Object $bodyObj
